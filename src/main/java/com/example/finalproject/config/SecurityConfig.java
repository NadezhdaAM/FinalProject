...
//                .anyRequest().authenticated()
                .requestMatchers("/auth/login", "/error","/auth/changePassword", "/auth/password/change","/auth/registration", "/product", "/product/info/{id}",
                ...
                .formLogin().loginPage("/auth/login") // указываем какой url запрос будет отправлятся при заходе на защищенные страницы
                ...
                .failureUrl("/auth/login") // Указываем куда необходимо перенаправить пользователя при проваленной аутентификации. В запрос будет передан объект error, который будет проверятся на форме и при наличии данного объекта в запросе выводится сообщение "Неправильный логин или пароль"
                .and()
